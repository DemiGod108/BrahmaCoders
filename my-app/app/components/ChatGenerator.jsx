...
const sendPrompt = async () => {
  if (!wallet) return alert("Connect wallet first"); // rely on passed prop
  ...
  try {
    ...
    if (res.ok) {
      ...
      aiMessage.content = `❌ ${data.error || "Something went wrong."}`;
    }
    ...
    setMessages((prev) => [
      ...prev,
      { role: "ai", content: "💀 Server error or no connection.", mode },
    ]);
    setLoading(false);
  }
};
          ...
          {messages.length === 0 && (
            <div className="text-center py-16 space-y-4">
              ...
              <p className="text-neutral-500 text-sm">
                Select a media type below and input your prompt to generate AI art.
              </p>
            </div>
          )}
              ...
              <div
                className={`max-w-[85%] md:max-w-[70%] rounded-2xl p-4 shadow-xl ${
                  msg.role === "user"
                    ...
                    : "bg-neutral-800/90 backdrop-blur-sm border border-neutral-700 text-neutral-200"
                }`}
              >
                {msg.role === "user" && (
                ...
                {msg.image && (
                  <img
                    src={msg.image}
                    alt="Generated"
                    className="mt-4 rounded-xl w-full shadow-lg"
                  />
                )}
                {msg.audio && (
                  <audio
                    ...
                    style={{ filter: "invert(1) grayscale(10%)" }}
                  />
                )}
                {msg.video && (
                  <video
              ...
              <div className="max-w-[70%] bg-neutral-800/90 backdrop-blur-sm rounded-2xl p-4 border border-neutral-700">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-neutral-400 text-sm">
                    Generating your {mode}... Please wait.
                  </span>
                </div>
                {mode === "music" && (
  ...
  placeholder={
    wallet
      ? getModePlaceholder()
      : "Connect wallet to start generating..."
  }
  className={`flex-1 bg-neutral-800/80 backdrop-blur-sm px-5 py-3.5 rounded-full outline-none text-sm md:text-base border border-neutral-700 focus:border-emerald-500 focus:bg-neutral-800 transition-all placeholder:text-neutral-500 text-white ${
    !wallet || loading ? "opacity-50 cursor-not-allowed" : ""
  }`}
...
<button
  ...
  className={`p-3.5 rounded-full font-medium text-sm transition-all flex items-center justify-center ${
    !wallet || loading || !input.trim()
      ? "bg-neutral-700 text-neutral-500 cursor-not-allowed"
      : "bg-emerald-500 text-black hover:bg-emerald-600 shadow-md shadow-emerald-500/30"
  }`}
>
